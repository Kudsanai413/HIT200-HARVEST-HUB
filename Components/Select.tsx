...
export default function Select({ options, size, master, dispatch, modal, action }: props) {
			...
			<Animated.FlatList
				...
				<Pressable
					...
					onPress={ () => {
						setSelected(item);
						if (dispatch){
							!action ? dispatch({ type: "type", payload: item }) : dispatch({type: action, payload: item});
						}
						setDropDown(false);
						if (modal){ modal(true) }
					}}
